...
describe 'Ubuntu 16.04 OS image', os_image: true do
  ...
  #it_behaves_like 'a Linux kernel 4.4.x based OS image'
  ...
  context 'installed by base_debootstrap' do
    ...
    describe file('/etc/lsb-release') do
      ...
      it { should contain 'DISTRIB_CODENAME=xenial' }
    end
  ...
  describe 'base_apt' do
    describe file('/etc/apt/sources.list') do
      #if Bosh::Stemcell::Arch.ppc64le? rishi
      if Bosh::Stemcell::Arch.s390x?
        it { should contain 'deb http://ports.ubuntu.com/ubuntu-ports/ xenial main restricted' }
        it { should contain 'deb http://ports.ubuntu.com/ubuntu-ports/ xenial-updates main restricted' }
        it { should contain 'deb http://ports.ubuntu.com/ubuntu-ports/ xenial universe' }
        it { should contain 'deb http://ports.ubuntu.com/ubuntu-ports/ xenial-updates universe' }
        it { should contain 'deb http://ports.ubuntu.com/ubuntu-ports/ xenial multiverse' }
        it { should contain 'deb http://ports.ubuntu.com/ubuntu-ports/ xenial-updates multiverse' }

        it { should contain 'deb http://ports.ubuntu.com/ubuntu-ports/ xenial-security main restricted' }
        it { should contain 'deb http://ports.ubuntu.com/ubuntu-ports/ xenial-security universe' }
        it { should contain 'deb http://ports.ubuntu.com/ubuntu-ports/ xenial-security multiverse' }
  ...
  context 'installed by base_ubuntu_packages' do
    ...
    %w(
      ...
      wget
      zip
    ).reject{ |pkg| Bosh::Stemcell::Arch.s390x? and ( pkg == 'rsyslog-mmjsonparse' or pkg == 'rsyslog-gnutls' or pkg == 'rsyslog-relp') }.each do |pkg|
      describe package(pkg) do
        it { should be_installed }
      end
    end
